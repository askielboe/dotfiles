...
# Center: Cold Turkey Micromanager focus-block timer + allowlisted-app glyphs.
# Sourced by ../sketchybarrc, right after pomodoro. Hidden unless a block is
# active. State comes from the app's prefs plist (not nix-managed, no CLI/events)
...
sketchybar --add item micromanager center \
  --set micromanager \
    icon=󰒃 \
    icon.color="$MAUVE" \
...
for i in 1 2 3 4 5 6; do
  sketchybar --add item "micromanager.app.$i" center \
